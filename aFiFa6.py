...
class Lover(telepot.helper.ChatHandler):
    ...
    def on_chat_message(self, msg):
        ...
        #== Bagian Beri Pakan ==
            ...
            sent = self.sender.sendMessage('Pemberian Pakan pada Kolam 1 sedang dilakukan...')
            #motor servo gerak, merekam
            self._editor = telepot.helper.Editor(self.bot, sent)
            self._edit_msg_ident = telepot.message_identifier(sent)
        elif (command == 'Pakan Kolam 2'):
            sent = self.sender.sendMessage('Pemberian Pakan pada Kolam 2 sedang dilakukan...')
            #motor servo gerak, merekam
            self._editor = telepot.helper.Editor(self.bot, sent)
            self._edit_msg_ident = telepot.message_identifier(sent)
        elif (command == 'Pakan Kolam 3'):
            sent = self.sender.sendMessage('Pemberian Pakan pada Kolam 3 sedang dilakukan...')
            #motor servo gerak, merekam
            self._editor = telepot.helper.Editor(self.bot, sent)
            self._edit_msg_ident = telepot.message_identifier(sent)
        elif (command == 'Pakan Semua Kolam'):
            sent = self.sender.sendMessage('Pemberian Pakan pada Kolam 1, 2 dan 3 sedang dilakukan...')
            #motor servo gerak, merekam
            self._editor = telepot.helper.Editor(self.bot, sent)
            self._edit_msg_ident = telepot.message_identifier(sent)
        #== end Beri Pakan ==
        ...
        #== Bagian Status ==
            ...
            status += '-> Satatus = '
            sent = self.sender.sendMessage(status, parse_mode='html', reply_markup=self.keyboard7)
            self._editor = telepot.helper.Editor(self.bot, sent)
            self._edit_msg_ident = telepot.message_identifier(sent)
        #== end Bagian Status ==
        else :
